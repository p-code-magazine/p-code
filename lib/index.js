...
export class PCode {
  ...
  run(code, ctx = 'external', interval = -1) {
    this.unpack(code);
    ...
    this.runContext = ctx;
    if (this.runContext == 'internal' && !this.looper) {
      const iInterval = (interval <= 0) ? 1/30 : interval;
      this.isPlaying = true;
      this.looper = new Tone.Loop(_ => {
        this.internalLoop();
      }, iInterval).start(0);
